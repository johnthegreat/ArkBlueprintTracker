...
export default class BlueprintProvider {
	...
	getBlueprints(): Promise<Blueprint[]> {
		return apiHelper<Blueprint[]>(fetch(`${this.apiBaseUrl}/blueprint?_sort=itemName&_order=asc`));
	}
	...
	getBlueprintsWithSearchQuery(query?: string): Promise<Blueprint[]> {
		if (!query) {
			...
			return apiHelper<Blueprint[]>(fetch(`${this.apiBaseUrl}/blueprint?q=${encodeURIComponent(query)}&_sort=itemName&_order=asc`));
		}
	}

	createBlueprint(blueprint: Blueprint): Promise<Blueprint> {
		return apiHelper<Blueprint>(fetch(`${this.apiBaseUrl}/blueprint`, {
			method: 'POST',
			headers: {
