...
function getItemToMaterialsDictionary(arkItemMaterials: {Item: string, Materials: string[]}[]) {
...
const arkItemMaterialsArrayByItem = getItemToMaterialsDictionary(arkItemMaterials);
...
export default function AddBlueprintModal(props: {
	...
	const [errorAlertMessage, setErrorAlertMessage] = useState<string|undefined>(undefined);
	...
	const [formField_armorRating, setFormField_ArmorRating] = useState<string>('');
	const [formField_damageRating, setFormField_DamageRating] = useState<string>('');
	const [formField_durabilityRating, setFormField_DurabilityRating] = useState<string>('');
	...
	useEffect(function() {
		...
		if (props.existingBlueprint) {
			...
			if (props.existingBlueprint.materialCosts) {
				...
				setMaterialsCount(_getMaterialsCountDict(props.existingBlueprint.materialCosts));
			}
			setFormField_ArmorRating(props.existingBlueprint.armor?.toString() || '');
			setFormField_DamageRating(props.existingBlueprint.damage?.toString() || '');
			setFormField_DurabilityRating(props.existingBlueprint.durability?.toString() || '');
	...
	return (
		<Modal show={showModal} onHide={() => handleClose(false)}>
			...
			<Modal.Body>
				{errorAlertMessage ? <div className="alert alert-danger">{errorAlertMessage}</div> : <></>}
				<form id="createClusterModalForm needs-validation">
					<div className="mb-3">
						...
						<Typeahead
							...
							inputProps={!isBlueprintSelected() ? {"aria-describedby": "invalidSelectedBlueprint"} : {}}
						/>
					...
					<div className="mb-3">
						...
						<select required id="quality" className="form-select" value={itemQuality} onChange={(e) => setItemQuality(e.target.value)}>
					...
					{doesItemHaveArmor(selectedItem) ? <div className="mb-3">
						...
						<input type="number" step="0.1" min={25} id="armor" className="form-control" value={formField_armorRating} onChange={
							(e) => setFormField_ArmorRating(e.target.value)
						} />
					</div> : <></>}

					{doesItemHaveDamage(selectedItem) ? <div className="mb-3">
						<label htmlFor="damage" className="form-label"><strong>Damage</strong></label>
						<input type="number" step="0.1" min={100} max={298} id="damage" className="form-control" value={formField_damageRating} onChange={
							(e) => setFormField_DamageRating(e.target.value)
						} />
					</div> : <></>}
					...
					{doesItemHaveDurability(selectedItem) ? <div className="mb-3">
						...
						<input type="number" min={0} step="1.0" id="durability" className="form-control" value={formField_durabilityRating} onChange={
							(e) => setFormField_DurabilityRating(e.target.value)
						} />
					</div> : <></>}
						...
						{materials.map((material: string) => {
							return <div key={material} className="mb-3">
								<div className="row">
									...
									<div className="col-sm">
										<input id={"material["+material+"]"} type="number" min={0} className="w-100" value={materialsCount[material] || ''} onChange={
											(e) => {
